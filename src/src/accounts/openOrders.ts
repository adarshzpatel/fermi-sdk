...
import { EventType } from "./eventHeap";
...
export class OpenOrders {
  ...
  public async cancelOrder(
    ...
    if ("clientOrderId" in order) {
      ...
      const [ix] = await this.cancelOrderByClientId(id);
      ixs.push(ix);
    } else {
      const id = order.leafNode.key;
      const [ix] = await this.cancelOrderById(id);
      ixs.push(ix);
    }
  ...
  public async cancelOrderById(id: BN) {
    return this.market.client.cancelOrderById(
      this.pubkey,
      this.account,
      this.market.account,
      id,
  ...
  public async cancelOrderByClientId(id: BN) {
    return this.market.client.cancelOrderByClientId(
      this.pubkey,
      this.account,
      this.market.account,
      id,
  ...
  public async settleFundsIx(): Promise<[TransactionInstruction, Signer[]]> {
