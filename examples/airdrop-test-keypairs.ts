...
import { airdropToken, initClientWithKeypairPath } from "./utils";

import { FermiClient } from "../src";
import { PublicKey } from "@solana/web3.js";

const main = async () => {
  // const marketOwnerKp = getLocalKeypair("/Users/dm/.config/solana/id.json");
  const marketOwnerKp = OWNER_KEYPAIR;
  const bobClient: FermiClient = initClientWithKeypairPath(
    "./test-keypairs/bob/key.json"
  );
  const aliceClient: FermiClient = initClientWithKeypairPath(
